...
package     Creatures::DS::Netbabel::Packet::Auth;
use base qw(Creatures::DS::Netbabel::Packet);
use constant HEADER_LENGTH => 0x34; # Netbabel header length
...
my $VERSION = do { my @r = (q$Revision: 1.4 $ =~ /\d+/g); sprintf "%d."."%02d" x $#r, @r };
...
sub decode {
	...
	# doesn't work.
	$priv->{error_count}  = 0;
	$priv->{username_len} = $self->dec('username_len', 0x2c, 4, 'L');
	$priv->{password_len} = $self->dec('password_len', 0x30, 4, 'L');

#	$priv->{username} = substr(${$self->packet()}, 0x34, $priv->{username_len} -1);
#	$priv->{password} = substr(${$self->packet()}, 0x34+$priv->{username_len},
#	                                               $priv->{password_len}-1);
...
sub username {
	...
	if(defined $value) {
		$priv->{username}     = $value;
		$priv->{username_len} = length($value)+1;
	}
		
	if (not defined $priv->{username}) {
		$priv->{username} = substr(${$self->packet()}, 0x34, $priv->{username_len} -1);
	}
...
sub password {
	...
	if(defined $value) {
		$priv->{password}     = $value;
		$priv->{password_len} = length($value)+1;
	}

	if (not defined $priv->{password}) {
		$priv->{password} = substr(${$self->packet()}, 0x34+$priv->{username_len},
		                                               $priv->{password_len}-1);
	}
...
sub packet_length {
	...
	return $priv->{username_len} + $priv->{password_len} + HEADER_LENGTH;
}
...
##########################################################################
# $Log: Auth.pm,v $
# Revision 1.4  2003/04/14 18:13:41  vadim
# Username and password now aren't read on packet decoding.
#
# Revision 1.3  2003/04/14 17:41:25  vadim
...
#
# Revision 1.2  2003/04/14 15:28:37  vadim
# Added packet_length and min_header_length methods
#
# Revision 1.1  2003/04/08 16:26:34  vadim
# Added packet parser for username/password packets. Currently works.
#
